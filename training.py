...
noised_inp_ = tf.placeholder(dtype=tf.float32, shape=[None, None], name = 'model_input')
clear_ground_truth_ = tf.placeholder(dtype=tf.float32, shape=[None, None], name = 'ground_truth')
denoised_out_ = model.get_model(inp, settings['model_selection'])
...
if 1:
    ...
    loss_ = tf.reduce_mean(tf.square(clear_ground_truth_ - denoised_out), axis=[0,1])
elif 0:
    #loss = tf.losses.absolute_difference(clear_ground_truth, denoised_out)
    loss_ = tf.reduce_mean(tf.abs(clear_ground_truth_ - denoised_out), axis=[0,1])
...
# save settings
with open(os.path.join(save_path, 'training_info.txt')) as f:
...
for e in range(settings['epoch']):
    ...
    for step in range(settings['step_per_epoch']):
        x_train, y_train = train_generator.get_batch_data()
        ...
        loss_sum += loss
        print('epoch: {0}/{1}, step: {2}/{3}, loss: {4:.4f}'.format(e,settings['epoch'],
